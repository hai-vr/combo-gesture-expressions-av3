...
public class ComboGestureCompiler : MonoBehaviour
{
    ...
    private void CreateOrReplaceController(AnimationClip emptyClip)
    {
        ...
        for (var left = 0; left < 8; left++)
        {
            for (var right = left; right < 8; right++)
            {
                ...
                {
                    new Parameter {name = HaiGestureComboParamName, value = left * 10 + right}
                };
    ...
    private void CreateOrReplaceBlinkingOverrideView(AnimationClip emptyClip)
    {
        ...
        var enableBlinking = CreateBlinkingState(machine, TrackingType.Tracking, emptyClip);
        var disableBlinking = CreateBlinkingState(machine, TrackingType.Animation, emptyClip);
    ...
    private static AnimatorState CreateBlinkingState(AnimatorStateMachine machine, TrackingType type,
        AnimationClip emptyClip)
    {
        var enableBlinking = machine.AddState(type == TrackingType.Tracking ? "EnableBlinking" : "DisableBlinking", GridPosition(type == TrackingType.Tracking ? 0 : 2, 3));
        enableBlinking.motion = emptyClip;
        enableBlinking.writeDefaultValues = false;
