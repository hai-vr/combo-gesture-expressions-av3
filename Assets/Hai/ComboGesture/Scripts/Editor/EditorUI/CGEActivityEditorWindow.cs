...
namespace Hai.ComboGesture.Scripts.Editor.EditorUI
{
    ...
    public class CgeEditorWindow : EditorWindow
    {
        ...
        private static readonly Color LeftSymmetricalBg;
        private static readonly Color RightSymmetricalBg;
        private static readonly Color InconsistentBg;
...
At the time this version has been published, generating the layer will break your Lipsync blendshapes.", MessageType.Error);
        ...
        private void DrawInner(string propertyPath, string oppositePath, bool partial = false)
        {
            ...
            if (usePermutations)
            {
                ...
                else if (property.objectReferenceValue == null && oppositeProperty.objectReferenceValue == null || isLeftHand && property.objectReferenceValue == null && oppositeProperty.objectReferenceValue != null)
                {
                    ...
                    DrawColoredBackground(isLeftHand ? LeftSymmetricalBg : RightSymmetricalBg);
                }
                ...
                {
                    DrawColoredBackground(InconsistentBg);
                }
                else
                {
            ...
            if (usePermutations && propertyPath != oppositePath && property.objectReferenceValue == oppositeProperty.objectReferenceValue && property.objectReferenceValue != null)
            {
                ...
                GUILayout.BeginArea(new Rect(10, PictureHeight - singleLineHeight * 1.75f, GuiSquareWidth - 10, singleLineHeight * 1.5f));
            ...
            {
                EditorGUI.BeginDisabledGroup(false);
                GUILayout.BeginArea(new Rect(GuiSquareWidth - 2 * singleLineHeight, PictureHeight - singleLineHeight * 1.75f, 2 * singleLineHeight, singleLineHeight * 1.5f));
                if (GUILayout.Button("↗↙"))
                {
                    Swap(propertyPath, oppositePath);
                }
        ...
        {
            var aProp = serializedObject.FindProperty(propertyPath);
            var bProp = serializedObject.FindProperty(oppositePath);
        ...
        {
            serializedObject.FindProperty(pathToClear).objectReferenceValue = null;
            serializedObject.ApplyModifiedProperties();
        }
